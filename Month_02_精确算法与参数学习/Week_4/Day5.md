...
前四天各做一件事：Day 1 加破缺、Day 2 加初解、Day 3 做诊断、Day 4 配预算。它们有一个共同的前提：**四种方法的结果必须落到同一张表上**。否则跨方法的比较、汇总、乃至本周要做的选择矩阵都无从谈起。
...
3. **这一行同时暴露了一条不好看但必须写进笔记的事实**：`milp_tight` 在 10 秒预算下给出 `543`，比 `heur_spt` 的 `532` **更差**。正确读法是「预算内的精确方法不保证优于一个规则解」——10 秒不够 CBC 在序列模型上搜出比 SPT 更好的解。这不是接口的问题，而是一条已被实测坐实的依据：**大规模 / 预算不足时先给规则解**。
...
统一接口的价值在「加方法不用改比较脚本」上兑现。接入一个新模型，只需要：
...
反过来说，只要这六件事都做到，月末批次的 `benchmark.py` 就能按名字调到它——**不需要改批次代码，也不需要改领域模型**。这正是「一行都不用改」的具体含义。

一个最小的骨架：

```python
@register("fjsp_cpsat", "FJSP：工序机器双重选择 + NoOverlap")

...
| `valid inequality` | 由一个**已验证可行解**推出 | 它只排除「比已知解更差」的区域 | Day 2 的目标上界割 `obj <= UB` |

三者的**共同点**是都不许改变最优值；**不同点**是合法性的论证方式完全不同：第一类靠置换群，第二类靠模型蕴含，第三类靠一个具体的可行解。混着叫「强化技巧」会导致后面判断失误——Day 2 的 `fixed_prefix` 会被不少资料也算作「warm start 技巧」，但它**不满足这条共同点**。
...
3. **`horizon` 是合法上界**，不是估计值：`_default_horizon` 取 `最晚释放时间 + Σp`，把全部工序串行排完也不会超过它。`horizon` 同时是变量的上界，因此它的大小直接进入第 3 节的系数跨度（Day 3 会把这件事量化）。

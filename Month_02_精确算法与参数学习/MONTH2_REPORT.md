...
同一实例在同一方法上扫描预算，节点数大幅增长而**界在小数点后 17 位完全不变**：

| 实例 / 方法 | 3 s | 10 s | 30 s |
|---|---|---|---|
| `single_20` / `milp_tight` | 界 65.50927046336038，31 节点 | 界 **同值**，817 节点 | 界 **同值**，3794 节点 |
| `parallel_24` / `cpsat_parallel` | 界 24.0，17206 冲突 | 界 **同值**，49641 冲突 | 界 **同值**，142997 冲突 |

`single_20` 上节点数增长 **122 倍**，界一动不动。原因是松弛太弱：分支产生的新节点 LP 界与根节点同级，全局下界被根界钉住。**把 3 秒加到 30 秒（10 倍预算）在这批实例上买不到任何 gap 改善**——这是「先换 formulation，再谈调预算」最直接的实测证据。
...
**本月没有生成任何图。** 全部证据以表格与 CSV 呈现（`results.csv` / `summary.csv` / `report.md`）。Week 4 Day 6 计划里的「性能分布」是用逐方法的 gap 与 wall-time 列表表达的，不是直方图。这是一个**明确的缺口**而不是「已覆盖」：M1 批次有三张 PNG，M2 没有对应的绘图脚本。补一个只读 `artifacts/month2/` 的绘图脚本不会改变本批次的源码指纹（绘图代码不在被 hash 的包内），列为后续工作。

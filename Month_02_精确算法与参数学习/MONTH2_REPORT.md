...
> **历史记录说明（笔记重写时补充）**：下文数值、测试数量、版本与性能结论保留自既有报告，本次文档重写未重新执行这些批次，也未复核“当前源码 hash 一致”等时点声明。理论学习请从[新月度索引](README.md)开始，并先读[概念勘误与实现边界](CONCEPT_CORRECTIONS.md)。尤其不要把固定变量子问题的最优状态/下界当成原问题证书，不要把有限对拍当成一般合法性证明；本报告中的旧概括须按勘误解释。

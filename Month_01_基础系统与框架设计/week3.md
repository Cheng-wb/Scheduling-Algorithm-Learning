...
一个**容易误判的事实**：该脚本用的实例是 `generate_instance(20, jobs=12, machines=1)`，它与 Week 4 基准集的 `single_12` **逐字节相同**（SHA-256 均为 `de904454fddb4a08…`）。所以这个「Week 3 演示」并不是另一个小玩具——它就是 §8 主实验里那张表所用的实例，脚本输出可以直接和 §8 的均值对照（脚本固定搜索 seed=0，取的是三个 seed 中的一个）。

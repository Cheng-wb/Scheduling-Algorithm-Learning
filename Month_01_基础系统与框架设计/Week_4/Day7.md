...
7. 说出 Month 2 会复用哪些既有模块、扩展哪些能力，以及为什么本月的 `best-known` gap 不能被当成求解器证明的 gap。
...
| 界（下界或松弛） | 本月未使用 | Month 2 的 MILP / CP-SAT bound |
...
### 4.3 接入下个月：Month 2 的复用与扩展

M2 的 MILP / CP-SAT 不需要重新发明输入与解：
...
这也解释了一个容易被忽略的事实：`routes_12` 的 `best-known = 40` 恰好由敏感性组的低温 SA 取到（`seed = 0` 那次）。**参考值来自被评价的方法之一**，所以「用 `gap` 评价低温 SA」在这个实例上等于自己给自己打分。这是 Month 2 引入独立 `bound` 的另一个理由。
...
脚本做七件事：打印验收证据表并现场核对批次产物、打印修正后的结果摘要（含两个 `optimum` 的现场重枚举）、给出复现命令清单与版本证据、列出局限、给出 Month 2 的复用与扩展、给闭卷自测建议、收尾。实际输出：
...
== 5. Month 2 会复用与扩展什么 ==
  直接复用（不重新发明）：
    ...
    - 同一个独立验证器：validate_schedule 不调用 decoder，精确方法也要过同一道闸门。
    - 同一套 Objective：Cmax / ΣCj / ΣTj / ΣwjCj / Lmax 由同一个模块计算。
    - 同一批输入与同一批基线结果：精确方法要在这个月的数据上对比，而不是换一批数据自证。

  扩展（这个月没有的能力）：
    - MILP / CP-SAT：在同样输入上给出 bound，并报告真实的求解器 gap。
    - status 词表扩展：从 BASELINE / BUDGET / LOCAL_OPTIMUM 增加到含 OPTIMAL / INFEASIBLE / TIME_LIMIT 的求解器状态。
    - 参考口径升级：这个月的 best-known gap 不能被当作求解器证明的 gap；
      只有在求解器给出 bound 与最优性证明之后，才能写 optimum。
    - 规模与资格分布扩展：更大的实例、更一般的机器资格，才有区分度。
...
5. 第 5 段明确了 Month 2 的交接纪律：复用 `Instance` / `Schedule` / 验证器 / `Objective`，扩展 `bound` 与 `status`，**不把本月的 `best-known` gap 当作求解器证明的 gap**。这条纪律决定了 M2 报告里第一次可以合法写 `optimum` 的时机。
...
**结论**：Month 2 的目标不是「用求解器跑一遍」，而是**在同一道题上补上本月缺的那一半证据**——下界与最优性证明。
...
统计补充：三个 `seed` 的均值差只作**探索性**观察（Day 6 第 7 节已经看到并列与 1 个单位就能翻转排名的情况）；更正式的结论需要更多实例、独立测试集、配对分析与不确定性估计。项目最终的价值不是某一批表或某一组参数，而是**把这些步骤变成可重复的过程**——这也正是 Month 2 要接着做的事。
...
| 问题 | 现状 | 下一步 |
|---|---|---|
| `best-known` 参考由被评价的方法取到，评价会自我强化 | `routes_12` 的 40 来自低温 SA | M2 用 `bound` 取代本批最小值 |
...
| SA 参数与目标尺度相关 | `exp(-Δ/T)` 的 Δ、T 同量纲 | M2 前做尺度归一化或改用相对温度 |
...
5. **练习 5（交接）**：写出 M2 接手时最需要先确认的三件事（输入模型、验证器、参考口径），并说明为什么 `best-known` gap 不能直接当求解器 gap。
...
- [ ] 能说出 Month 2 复用什么、扩展什么，以及 `best-known` gap 为什么不能当求解器 gap。
...
- Q10：Month 2 会复用哪些模块、扩展哪些能力？`best-known` gap 为什么不能被当作求解器证明的 gap？

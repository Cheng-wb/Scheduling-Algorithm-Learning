...
| 「均值 44.33 比 44.67 好，所以高温更好」 | 差 0.33，只有三个 `seed`，还可能是并列 | 「高温与快冷却在本批均值并列 44.33」 |
...
3. 第 5 段的三个空闲都给出了**具体原因**：两个来自作业释放时间（J002 的 `release_time=3`、J004 的 `release_time=1`），一个来自作业内先后顺序（机器 M2 上的 [39,40) 在等前驱工序结束）。**结论**：只要 decoder 是「追加到机器末尾」的，图上的每一段空闲都应该能这样归因；归因不了才需要怀疑 decoder 或数据。

...
它们的作用是：把「运行几轮」正式换算成可比较的**计算预算**——统一的配置与结果口径会**直接挂在这条链上**。
...
3. 复杂度：每步线性扫描 `remaining` 并 `remove`，总体约 O(n²)。M1 优先选择「可验证」而不是「增量高效」，增量评估不在本周范围。

`initial_candidate` 是同一文件的另一半：`p` 降序 + 平局按 `op.id` 升序排优先级，再为每道工序贪心选 `(max(load, release), machine.id)` 最小的机器。它既是 Week 1 的 LPT 基线，也是所有搜索算法的**共享初始解**。
...
| 8 | 表示冗余会让搜索结果不可比吗？ | 目前只影响「抽样分布」，不影响可行性；**本周结束时仍是开放问题** |

第 8 条是本周留下的开放问题：**「运行几轮」还没有被正式换算成可比较的计算预算，统一的配置与结果口径也还没有落地。** 而那个「一次评价」里包含的正是今天这条链上的三步——所以今天必须把链接死，预算才算得清。
...
> **Week 2 的真正成果不是多了一个解码器或验证器，而是把「输入不可变、邻域返回新对象、每次评价都要走完整链路」钉成了契约；同时诚实地记下四条边界——表示有冗余、追加解码不填空隙、生成器只有全资格实例、Oracle 只能证明单工序极小实例——搜索算法才有可比的成本账本和可信的落脚点。**

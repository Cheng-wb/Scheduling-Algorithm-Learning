...
验证包含 107 项测试、十个不同手算案例、三条精确解码轨迹、五个小实例枚举对拍和九类损坏排程。该 107 项是记录本批次时的收集数；其后补充的验收测试使当前代码库收集到 127 项（`python -m pytest --collect-only -q`），新增部分见 Week 2–4 笔记。正式实验共 162 次，全部成功；另有故障注入验证失败留痕。复现脚本逐次重算并核对目标、状态、评价数、候选、排程和完整轨迹。
...
**重要前提：复现 `month1_refactored` 需要当时的源码版本。** `reproduce.py` 的第一道闸门比较当前源码 hash 与 `metadata.json` 记录的 `source_sha256`；当前工作区并不匹配（记录 `025ed469…`，当前 `4d6d74ea…`），因此脚本会以 `source hash differs from recorded run` 明确拒绝，而不是给出一个看似成功的假复现。原因记录在 `metadata.json` 自身：该批次的 `working_tree_dirty` 为 `true`，即产自未提交的工作区，其源码既非记录 commit 对应版本、也非当前版本。要在当前代码上获得可复现的批次，请用上面的 benchmark 命令创建**新目录**重跑；此后该新批次的 `source_sha256` 即可核对通过。

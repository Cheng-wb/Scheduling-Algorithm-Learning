...
还有一个容易被忽略的事实：Best 并不是终点，而是其他方法的零件——需要很多起点、或者每次只想评价一个候选的方法，基础搜索会选 **First** 而不是 Best（因为更便宜）。**先看清 Best 的成本结构，才知道什么时候不该用它。**
...
返回值是 `best`，不是 `current`（Day 1 第 3 节的结论在这里第一次显出必要性）。
...
**所以「接受率」这个词在三种算法里根本不是同一个量。** First/Best 的一个 `accepted=True` 代表一整轮（可能 149 次评价）的结果；SA 的一个 `accepted=True` 代表**一次**提议的接受事件。把两者的 `accepted` 混在一起算接受率，等于用「轮」除以「次」，是纯粹的统计口径错误。计算 SA 接受率时绝不能混入 Best 的轨迹。
...
而 SA 的接受准则是 `delta <= 0 or rng.random() < exp(...)`，**等值候选（Δ=0）无条件接受**。所以：
...
3. `best` 的 `current` 列在**所有**算法里都是「回填后」的值，读取轨迹做逐行分析时必须记住这条例外。

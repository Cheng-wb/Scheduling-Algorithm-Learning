...
复杂度：每步线性扫描候选列表并移除元素，总体约 `O(n²)`，另有输入与候选校验开销。M1 阶段优先保持**可验证**；增量评估不在本周范围。
...
| # | 问题 | 状态 |
|---|---|---|
| 1 | 所有搜索方法如何共享同一套评价预算？ | 本周结束时仍是开放问题 |
| 2 | 邻域的存储成本随 `n²` 增长怎么办？ | 本周结束时仍是开放问题 |
| 3 | 多工序实例能否被搜索流水线覆盖？ | 本周结束时仍是开放问题 |
| 4 | 枚举只能覆盖极小实例，大实例的参考值从哪来？ | 本周结束时仍是开放问题 |

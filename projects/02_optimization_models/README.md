...
**M2 四周全部完成。** 已实现生产计划 / 运输 / 指派 LP 与**独立构造的对偶**、单机序列 MILP（tight / loose Big-M）与时间索引替代模型、CP-SAT 区间模型（并行机 / JSP / Cumulative）、对称破缺与冗余约束、hint / 目标上界割 / 前缀固定、数值缩放体检与不可行冲突定位，以及跨方法的可复现批次。

月末产出为 **Optimization Model Lab**：一个 59 次运行的正式批次（零失败、零独立验证失败、三次独立运行零差异）、一个 29 次运行的 Week 4 工程实验、232 项测试、四个周报与 28 篇日笔记。入口见 [月度报告](../../Month_02_精确算法与参数学习/MONTH2_REPORT.md)。

三条最值得记住的实测结论：

1. **formulation 的结构比 Big-M 的紧松重要得多** —— 单机 ΣT 上两个 disjunctive 变体的根 LP 界都是 0，而时间索引模型的根界达到最优值的 98.75%–99.97%，20 作业 0.68 秒证明最优而序列模型 30 秒仍有 0.93 的 gap。
2. **更多时间不必然改善界** —— 节点数增长 122 倍，界在小数点后 17 位完全不变。
3. **同一个技巧在不同规模上可以结论相反** —— 对称破缺在 6–10 作业上是负收益，在 24 作业上把 30 秒未解实例变成 0.46 秒的已证明最优。

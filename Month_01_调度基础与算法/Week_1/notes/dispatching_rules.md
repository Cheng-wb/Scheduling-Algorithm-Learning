...
> 来源：原第二周 Day 5。按主题归档；旧实验数字仍对应文中配置，不代表新计划的未完成验收已通过。
...
| 模块 | 职责 |
| --- | --- |
| [heuristics/rules.py](../../../projects/scheduling_basics/scheduling/heuristics/rules.py) | 五种排序键与优先级类型 |
| [heuristics/single_machine.py](../../../projects/scheduling_basics/scheduling/heuristics/single_machine.py) | 静态、动态排程以及规则便捷入口 |
| [heuristics/__init__.py](../../../projects/scheduling_basics/scheduling/heuristics/__init__.py) | 导出调用接口 |
| [dispatching_rules.py](../../../projects/scheduling_basics/experiments/dispatching_rules.py) | 数据集、规则比较、打印、MILP 基准 |
...
```powershell
python -m experiments.dispatching_rules
```

只运行规则、无需 OR-Tools：

```powershell
python -m experiments.dispatching_rules --skip-milp
```

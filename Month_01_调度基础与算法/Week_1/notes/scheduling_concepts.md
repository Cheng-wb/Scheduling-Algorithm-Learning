...
> 来源：原第二周 Day 1。按主题归档；旧实验数字仍对应文中配置，不代表新计划的未完成验收已通过。
...
└── experiments/
    ├── __init__.py
    └── model_basics.py        # 题目数据、手工排程、运行和打印
```
...
| 模块 | 职责 | 主要接口 |
| --- | --- | --- |
| [models.py](../../../projects/scheduling_basics/scheduling/models.py) | 定义输入任务、单条分配和完整排程 | `Job`、`ScheduledJob`、`Schedule` |
| [scheduler.py](../../../projects/scheduling_basics/scheduling/scheduler.py) | 根据给定顺序和释放时间生成排程 | `schedule_sequence(sequence, jobs)` |
| [metrics.py](../../../projects/scheduling_basics/scheduling/metrics.py) | 独立计算每个指标 | `makespan`、`tardiness`、`weighted_tardiness` 等 |
| [evaluator.py](../../../projects/scheduling_basics/scheduling/evaluator.py) | 校验排程并统一调用指标函数 | `evaluate(schedule)` |
| [model_basics.py](../../../projects/scheduling_basics/experiments/model_basics.py) | 准备题目、组合调用、打印结果 | `main()` |
...
```powershell
python -m experiments.model_basics
```
...
调用示例（在 scheduling_basics 项目目录 下）：
...
运行本节题目后，把结果与第 6、7 节手算表对照。日常练习仍以运行题目、输出结果为主。

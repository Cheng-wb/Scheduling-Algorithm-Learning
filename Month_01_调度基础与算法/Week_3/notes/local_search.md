...
> 来源：原第三周 Day 3。按主题归档；旧实验数字仍对应文中配置，不代表新计划的未完成验收已通过。
...
`search/local_search.py` 保存两种搜索入口；`search/neighborhood.py` 提供邻域；`evaluation/objective.py` 定义目标；`experiments/local_search_comparison.py` 组合初始规则、邻域和策略，输出实验结果。实验按天存放，核心模块可复用。

## 17. 核心结论

局部搜索通过“枚举、选择、更新、重复”改善初始解。严格接受规则决定目标值单调下降；起点、邻域、策略及并列处理共同决定路径和停止点。对结果的描述应同时说明目标、邻域和停止原因。

## 18. 与 Random Restart 的衔接

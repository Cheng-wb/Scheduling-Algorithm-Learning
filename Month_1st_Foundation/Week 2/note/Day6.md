...
[heuristics/parallel_machine.py](../scheduling/heuristics/parallel_machine.py) 提供三个接口：
...
[day6_parallel_machine.py](../experiments/day6_parallel_machine.py) 负责案例、下界、调用、按机器打印加工区间、负载和比较表。默认每个 MILP 实例限时 5 秒，可通过参数调整。

在 Week 2 目录运行：

```powershell
python -m experiments.day6_parallel_machine
python -m experiments.day6_parallel_machine --scale
python -m experiments.day6_parallel_machine --skip-milp
python -m experiments.day6_parallel_machine --scale --time-limit-ms 30000
```
